...
int QrHttpServer::processRequest(char *request, SOCKET & sock)
{
    ...
    // When key exist - operation won't be exucuted
...
int QrHttpServer::mHttpProcessRequest(char *request)
{
    ...
    // webclient requests logo of browser page
    if (strstr(firstString, requestLogo))
        return mLoadLogo();
    ...
    for (unsigned long long i = 0; i < mScreenShot.size(); i++)
    {
        ...
        // can't make like tmpRequestFile.str().c_str(); It doesn't work, why ?
        tmp = tmpRequestFile.str();
...
int QrHttpServer::mSendFile(const std::string & pathToFile) const
{
    ...
    if (send(mCurrentSocket, response.str().c_str(),
             (int)response.str().length(), 0) < 0)
    {
        return QR_HTTP_ERROR;
    }
...
int QrHttpServer::mLoadLogo() const
{
    ...
    {
        std::cout << "Can't load logo" << std::endl;
        // who needs logo ? nobody !
        // return QR_HTTP_ERROR;
    }
...
void QrHttpServer::mConstructBody()
{
    mBrowserBody +=
            ...
            "<i>Click on the image to download it:<p></i>" +
...
std::string QrHttpServer::mConstructFile(std::string fileName) const
{
    std::string browserFile =
            ...
            "<img src=\"" + fileName +
            "\" alt=\"ScanScreen\" width=\"640\" height=\"360\">"
            "</a>";
...
int QrHttpServer::mTestLoadPage(void)
{
    ...
    std::string temp = std::string("var wsUri = \"ws://") +
            std::string(st.ip) + ":" + std::to_string(st.port) +
            std::string("/chat\";");
...
int QrHttpServer::mConfirmWebSocket(const char *request)
{
    ...
    mWebSocketConnection = "Sec-WebSocket-Accept: " +
